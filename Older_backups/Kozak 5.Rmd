...
```{r Import and filter the first 3 replicates of the STIM1 Kozak data}
...
#ggplot() + geom_histogram(data = klib_comb, aes(x = log10_count))
...
```{r Bringing in replicates 4 and 5 performed with NextSeq}
...
kr4_none <- rbind(i0710, i0711)
kr4_none$kozak_sequence <- substr(kr4_none$Sequence, 1, 6)
kr4_none$coding <- substr(kr4_none$Sequence,7,20)
kr4_none_intended <- kr4_none %>% filter(coding == "ATGGATGTATGCG" & Sequence != "CATTGTATGGATGTATGCG") 
kr4_none_other <- kr4_none %>% filter(coding != "ATGGATGTATGCG") %>% mutate(kozak_sequence = "_OTHER")
kr4_none_template <- kr4_none %>% filter(Sequence == "GTTTTTATGGATGTATGCG") %>% mutate(kozak_sequence = "_TEMPL")
kr4_none <- rbind(kr4_none_intended, kr4_none_other, kr4_none_template)
kr4_none <- kr4_none %>% group_by(kozak_sequence) %>% summarize(Count = sum(Count), log10_count = log10(Count))
kr4_none_density <- ggplot() + geom_density(data = kr4_none, aes(x = log10_count))
kr4_none_density_table <- ggplot_build(kr4_none_density)$data[[1]]
kr4_none_minquant <- kr4_none_density_table %>% filter(x < quantile(kr4_none_density_table$x, minquant_fraction))
kr4_none_density_minima <- kr4_none_minquant[kr4_none_minquant$y == min(kr4_none_minquant$y),"x"]
kr4_none_density <- ggplot() + geom_density(data = kr4_none, aes(x = log10_count)) + geom_vline(xintercept = kr4_none_density_minima)
kr4_none_filtered <- kr4_none %>% filter(log10_count > kr4_none_density_minima)

kr4_cells <- rbind(i0712, i0713, i0714, i0715)
kr4_cells$kozak_sequence <- substr(kr4_cells$Sequence, 1, 6)
kr4_cells$coding <- substr(kr4_cells$Sequence,7,20)
kr4_cells_intended <- kr4_cells %>% filter(coding == "ATGGATGTATGCG" & Sequence != "CATTGTATGGATGTATGCG") 
kr4_cells_other <- kr4_cells %>% filter(coding != "ATGGATGTATGCG") %>% mutate(kozak_sequence = "_OTHER")
kr4_cells_template <- kr4_cells %>% filter(Sequence == "GTTTTTATGGATGTATGCG") %>% mutate(kozak_sequence = "_TEMPL")
kr4_cells <- rbind(kr4_cells_intended, kr4_cells_other, kr4_cells_template)
kr4_cells <- kr4_cells %>% group_by(kozak_sequence) %>% summarize(Count = sum(Count), log10_count = log10(Count))
kr4_cells_density <- ggplot() + geom_density(data = kr4_cells, aes(x = log10_count))
kr4_cells_density_table <- ggplot_build(kr4_cells_density)$data[[1]]
kr4_cells_minquant <- kr4_cells_density_table %>% filter(x < quantile(kr4_cells_density_table$x, minquant_fraction))
kr4_cells_density_minima <- kr4_cells_minquant[kr4_cells_minquant$y == min(kr4_cells_minquant$y),"x"]
kr4_cells_density <- ggplot() + geom_density(data = kr4_cells, aes(x = log10_count)) + geom_vline(xintercept = kr4_cells_density_minima)
kr4_cells_filtered <- kr4_cells %>% filter(log10_count > kr4_cells_density_minima)
...
kr5_none <- rbind(i0716, i0717)
kr5_none$kozak_sequence <- substr(kr5_none$Sequence, 1, 6)
kr5_none$coding <- substr(kr5_none$Sequence,7,20)
kr5_none_intended <- kr5_none %>% filter(coding == "ATGGATGTATGCG" & Sequence != "CATTGTATGGATGTATGCG") 
kr5_none_other <- kr5_none %>% filter(coding != "ATGGATGTATGCG") %>% mutate(kozak_sequence = "_OTHER")
kr5_none_template <- kr5_none %>% filter(Sequence == "GTTTTTATGGATGTATGCG") %>% mutate(kozak_sequence = "_TEMPL")
kr5_none <- rbind(kr5_none_intended, kr5_none_other, kr5_none_template)
kr5_none <- kr5_none %>% group_by(kozak_sequence) %>% summarize(Count = sum(Count), log10_count = log10(Count))
kr5_none_density <- ggplot() + geom_density(data = kr5_none, aes(x = log10_count))
kr5_none_density_table <- ggplot_build(kr5_none_density)$data[[1]]
kr5_none_minquant <- kr5_none_density_table %>% filter(x < quantile(kr5_none_density_table$x, minquant_fraction))
kr5_none_density_minima <- kr5_none_minquant[kr5_none_minquant$y == min(kr5_none_minquant$y),"x"]
kr5_none_density <- ggplot() + geom_density(data = kr5_none, aes(x = log10_count)) + geom_vline(xintercept = kr5_none_density_minima)
kr5_none_filtered <- kr5_none %>% filter(log10_count > kr5_none_density_minima)

kr5_cells <- rbind(i0718, i0719, i0720, i0721)
kr5_cells$kozak_sequence <- substr(kr5_cells$Sequence, 1, 6)
kr5_cells$coding <- substr(kr5_cells$Sequence,7,20)
kr5_cells_intended <- kr5_cells %>% filter(coding == "ATGGATGTATGCG" & Sequence != "CATTGTATGGATGTATGCG") 
kr5_cells_other <- kr5_cells %>% filter(coding != "ATGGATGTATGCG") %>% mutate(kozak_sequence = "_OTHER")
kr5_cells_template <- kr5_cells %>% filter(Sequence == "GTTTTTATGGATGTATGCG") %>% mutate(kozak_sequence = "_TEMPL")
kr5_cells <- rbind(kr5_cells_intended, kr5_cells_other, kr5_cells_template)
kr5_cells <- kr5_cells %>% group_by(kozak_sequence) %>% summarize(Count = sum(Count), log10_count = log10(Count))
kr5_cells_density <- ggplot() + geom_density(data = kr5_cells, aes(x = log10_count))
kr5_cells_density_table <- ggplot_build(kr5_cells_density)$data[[1]]
kr5_cells_minquant <- kr5_cells_density_table %>% filter(x < quantile(kr5_cells_density_table$x, minquant_fraction))
kr5_cells_density_minima <- kr5_cells_minquant[kr5_cells_minquant$y == min(kr5_cells_minquant$y),"x"]
kr5_cells_density <- ggplot() + geom_density(data = kr5_cells, aes(x = log10_count)) + geom_vline(xintercept = kr5_cells_density_minima)
kr5_cells_filtered <- kr5_cells %>% filter(log10_count > kr5_cells_density_minima)
...
kr4_none_freq = kr4_none_filtered %>% mutate(n4a = Count) %>% mutate(f4a = Count/sum(Count))
kr4_cells_freq = kr4_cells_filtered %>% mutate(n4b = Count) %>% mutate(f4b = Count/sum(Count)) 
kr5_none_freq = kr5_none_filtered %>% mutate(n5a = Count) %>% mutate(f5a = Count/sum(Count)) 
kr5_cells_freq = kr5_cells_filtered %>% mutate(n5b = Count) %>% mutate(f5b = Count/sum(Count))
```


```{r Calculate STIM1 frequencies and enrichment scores for the first three replicates}
k_enrich <- merge(klib_freq[,c("kozak_sequence","f0")], kr1_freq[,c("kozak_sequence","f1")], by = "kozak_sequence", all = T)
k_enrich <- merge(k_enrich, kr2_freq[,c("kozak_sequence","f2")], by = "kozak_sequence", all = T)
k_enrich <- merge(k_enrich, kr3_freq[,c("kozak_sequence","f3")], by = "kozak_sequence", all = T)
k_enrich <- merge(k_enrich, kr4_none_freq[,c("kozak_sequence","f4a")], by = "kozak_sequence", all = T)
k_enrich <- merge(k_enrich, kr4_cells_freq[,c("kozak_sequence","f4b")], by = "kozak_sequence", all = T)
k_enrich <- merge(k_enrich, kr5_none_freq[,c("kozak_sequence","f5a")], by = "kozak_sequence", all = T)
k_enrich <- merge(k_enrich, kr5_cells_freq[,c("kozak_sequence","f5b")], by = "kozak_sequence", all = T)

k_enrich2 <- k_enrich %>% mutate(r1_score = f1 / f0, r2_score = f2 / f0, r3_score = f3 / f0, r4_score = f4b / f4a, r5_score = f5b / f5a)


k_enrich2$survival <- rowMeans(k_enrich2[,c("r1_score","r2_score","r3_score", "r4_score","r5_score")], na.rm = T)
k_enrich2$log10_survival <- log10(k_enrich2$survival)
...
```{r Calculate STIM1 frequencies and enrichment scores for the first three replicates}
k_score <- k_enrich3 %>% select(kozak_sequence, code, variant, survival, log10_survival, r1_score, r2_score, r3_score, r4_score, r5_score)
...
STIM1_Kozak_histogram <- ggplot() + 
  ...
  geom_histogram(data = k_score_samples, aes(x= log10_survival), bins = 10, color = "black", fill = "grey75") + 
...
kr4_plasmid <- rbind(i0710, i0711) %>% group_by(Sequence) %>% summarize(Count = sum(Count))
kr4_cells <- rbind(i0712, i0713, i0714, i0715) %>% group_by(Sequence) %>% summarize(Count = sum(Count))
kr4 <- merge(kr4_plasmid, kr4_cells,  by = "Sequence", all = T)



kr4 <- rbind(i0710, i0711, i0712, i0713, i0714, i0715) %>% group_by(Sequence) %>% summarize(Count = sum(Count))
kr4$kozak_sequence <- substr(kr4$Sequence,1,6)
kr4$coding <- substr(kr4$Sequence,7,20)
kr4_most <- kr4 %>% filter(coding == "ATGGATGTATGCG" & Sequence != "CATTGTATGGATGTATGCG")
kr4_other <- kr4 %>% filter(coding != "ATGGATGTATGCG")
kr4_template <- kr4 %>% filter(Sequence == "GTTTTTATGGATGTATGCG")
...
i0720 <- read.csv(file = "Data/STIM1_Kozak/I0720.csv", header = T)
i0721 <- read.csv(file = "Data/STIM1_Kozak/I0721.csv", header = T)
kr5 <- rbind(i0716, i0717, i0718, i0719, i0720, i0721) %>% group_by(Sequence) %>% summarize(Count = sum(Count))
kr5$kozak_sequence <- substr(kr5$Sequence,1,6)
kr5$coding <- substr(kr5$Sequence,7,20)
kr5_most <- kr5 %>% filter(coding == "ATGGATGTATGCG" & Sequence != "CATTGTATGGATGTATGCG") 
kr5_other <- kr5 %>% filter(coding != "ATGGATGTATGCG")
kr5_template <- kr5 %>% filter(Sequence == "GTTTTTATGGATGTATGCG")

ggplot() + geom_histogram(data = kr4, aes(x = log10(Count)))
ggplot() + geom_histogram(data = kr4, aes(x = log10(Count)))

kr4_comb <- rbind(data.frame("kozak_sequence" = c("_OTHER","_TEMPL"), "Count" = c(sum(kr5_other$Count),kr5_template$Count)),kr4_most[,c("kozak_sequence","Count")]) %>% mutate(log10_count = log10(Count))
kr5_comb <- rbind(data.frame("kozak_sequence" = c("_OTHER","_TEMPL"), "Count" = c(sum(kr5_other$Count),kr5_template$Count)),kr5_most[,c("kozak_sequence","Count")]) %>% mutate(log10_count = log10(Count))

kr4_density <- ggplot() + geom_density(data = kr4_comb, aes(x = log10_count))
kr4_density_table <- ggplot_build(kr4_density)$data[[1]]
kr4_minquant <- kr4_density_table %>% filter(x < quantile(kr4_density_table$x, minquant_fraction))
kr4_density_minima <- kr4_minquant[kr4_minquant$y == min(kr4_minquant$y),"x"]
kr4_density <- ggplot() + geom_density(data = kr4, aes(x = log10_count)) + geom_vline(xintercept = kr4_density_minima)
kr4_filtered <- kr4_comb %>% filter(log10_count > kr4_density_minima)

kr5_density <- ggplot() + geom_density(data = kr5_comb, aes(x = log10_count))
kr5_density_table <- ggplot_build(kr5_density)$data[[1]]
kr5_minquant <- kr5_density_table %>% filter(x < quantile(kr5_density_table$x, minquant_fraction))
kr5_density_minima <- kr5_minquant[kr5_minquant$y == min(kr5_minquant$y),"x"]
kr5_density <- ggplot() + geom_density(data = kr5, aes(x = log10_count)) + geom_vline(xintercept = kr5_density_minima)
kr5_filtered <- kr5_comb %>% filter(log10_count > kr5_density_minima)


kr4_freq = kr4_filtered %>% mutate(n4 = Count) %>% mutate(f4 = Count/sum(Count))
kr5_freq = kr5_filtered %>% mutate(n5 = Count) %>% mutate(f5 = Count/sum(Count))

k_enrich <- merge(k_enrich, kr4_freq[,c("kozak_sequence","n4","f4")], by = "kozak_sequence", all = T)
k_enrich <- merge(k_enrich, kr5_freq[,c("kozak_sequence","n5","f5")], by = "kozak_sequence", all = T)
...
```{r Bringing in STIM1 Kozak replicate data for the remaining two replicates performed by NextSeq}
i0710 <- read.csv(file = "Data/STIM1_Kozak/I0710.csv", header = T)
i0711 <- read.csv(file = "Data/STIM1_Kozak/I0711.csv", header = T)
...
```{r This replicate had different timepoints thus requiring a slightly different analysis}
...
```{r Bringing in additional replicate data}
i0710 <- read.csv(file = "Data/STIM1_Kozak/I0710.csv", header = T)
i0711 <- read.csv(file = "Data/STIM1_Kozak/I0711.csv", header = T)
i0712 <- read.csv(file = "Data/STIM1_Kozak/I0712.csv", header = T)
i0713 <- read.csv(file = "Data/STIM1_Kozak/I0713.csv", header = T)
...
```{r Separting based on STIM1 variant}

#k_enrich[is.na(k_enrich)] <- 0
k_enrich2 <- k_enrich %>% mutate(f1a = f1/f0, f2a = f2/f0, f3a = f3/f0)

for(x in 1:nrow(k_enrich2)){
  for(y in 14:ncol(k_enrich2)){
    if(!is.na(k_enrich2[x,y]) & k_enrich2[x,y] == "Inf"){k_enrich2[x,y] <- NA}
    if(is.na(k_enrich2[x,y])){k_enrich2[x,y] <- NA}
  }
}

k_enrich2$enrichment <- 10^((log10(k_enrich2$f1a) + log10(k_enrich2$f2a) + log10(k_enrich2$f3a)) / 3)
...
abund_raw = complete_frame5[,c("sequence","rf_predict_mfi","calibrated_score")]
abund_raw = abund_raw %>% mutate(kozak_sequence = substr(sequence, 1, 6))
abund_stim = abund_raw %>% mutate(code = substr(kozak_sequence, 1,1)) %>% filter(code != "T")
k_score = left_join(k_score, abund_stim)
...
STIM1_geom_smooth_plot <- ggplot() + theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank()) + 
  ...
  geom_smooth(data = k_score_samples, aes(x= calibrated_score, y= 10^survival, color = variant), alpha = 0.1, span = 2) +
  #facet_grid(cols = vars(variant)) +
  NULL; STIM1_geom_smooth_plot
ggsave(file = "Plots/STIM1_geom_smooth_plot.pdf", STIM1_geom_smooth_plot, height = 1.5, width = 2.9)
