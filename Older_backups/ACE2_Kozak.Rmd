...
```{r Looking for outliers between the Noderer | RF-imputed and the calibrated scores}

query_subset <- subset(complete_frame4) %>% filter(calibrated_score / rf_predict_mfi < 0.1 & calibrated_score / noderer^5 < 0.00000000001)

qlogo_dataframe <- data.frame("n6" = rep(0,4),"n5" = rep(0,4),"n4" = rep(0,4),"n3" = rep(0,4),"n2" = rep(0,4),"n1" = rep(0,4))
rownames(qlogo_dataframe) <- c("A","C","G","T")

for(x in 1:nrow(query_subset)){
  temp_seq <- substr(query_subset$sequence[x],1,6)
  qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,1,1),"n6"] <- qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,1,1),"n6"] + 1
  qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,2,2),"n5"] <- qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,2,2),"n5"] + 1
  qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,3,3),"n4"] <- qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,3,3),"n4"] + 1
  qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,4,4),"n3"] <- qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,4,4),"n3"] + 1
  qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,5,5),"n2"] <- qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,5,5),"n2"] + 1
  qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,6,6),"n1"] <- qlogo_dataframe[rownames(qlogo_dataframe) == substr(temp_seq,6,6),"n1"] + 1
}
qlogo_dataframe2 <- qlogo_dataframe / colSums(qlogo_dataframe)
qlogo_matrix <- as.matrix(qlogo_dataframe2)

## Function for working out the position weight matrix value
qlogo <- function(freq, total, bg=0.25){
  #using the formulae above
  p <- (freq + (sqrt(total) * 1/4)) / (total + (4 * (sqrt(total) * 1/4)))
  log2(p/bg)
}
qlogo_matrix2 <- qlogo(qlogo_matrix,6)
...
## Bar chart
qlogo_matrix_melt <- melt(qlogo_matrix)

qlogo_barplot <- ggplot() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  labs(x = "Position along Kozak", y = "Weighted\nfrequency") + 
  geom_bar(data = qlogo_matrix_melt, aes(x = Var2, y = value, fill = Var1), stat = "identity", color = "black") +
  NULL; qlogo_barplot
ggsave(file = "plots/qlogo_barplot.pdf", qlogo_barplot, height = 1.2, width = 3)

qlogo_pointplot <- ggplot() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "top") + 
  labs(x = "Position along Kozak", y = "Weighted\nfrequency") + 
  geom_hline(yintercept = 0.25, linetype = 2, alpha = 0.4) + 
  geom_point(data = qlogo_matrix_melt, aes(x = Var2, y = value, color = Var1, fill = Var1), stat = "identity", shape = 21, size = 2, alpha = 0.5) +
  NULL; qlogo_pointplot
ggsave(file = "plots/qlogo_pointplot.pdf", qlogo_pointplot, height = 2, width = 3.4)

```
```{r Trying to quantitate repeated nucleotides}
complete_frame4_repeats <- complete_frame4

for(x in 1:nrow(complete_frame4_repeats)){
  pair1 <- substr(complete_frame4_repeats$sequence[x],1,1) == substr(complete_frame4_repeats$sequence[x],2,2)
  pair2 <- substr(complete_frame4_repeats$sequence[x],2,2) == substr(complete_frame4_repeats$sequence[x],3,3)
  pair3 <- substr(complete_frame4_repeats$sequence[x],3,3) == substr(complete_frame4_repeats$sequence[x],4,4)
  pair4 <- substr(complete_frame4_repeats$sequence[x],4,4) == substr(complete_frame4_repeats$sequence[x],5,5)
  pair5 <- substr(complete_frame4_repeats$sequence[x],5,5) == substr(complete_frame4_repeats$sequence[x],6,6)
  complete_frame4_repeats$nt_repeats[x] <- sum(pair1, pair2, pair3, pair4, pair5)
  dint1 <- substr(complete_frame4_repeats$sequence[x],1,2) == substr(complete_frame4_repeats$sequence[x],3,4)
  dint2 <- substr(complete_frame4_repeats$sequence[x],2,3) == substr(complete_frame4_repeats$sequence[x],4,5)
  dint3 <- substr(complete_frame4_repeats$sequence[x],3,4) == substr(complete_frame4_repeats$sequence[x],5,6)
  complete_frame4_repeats$dint_repeats[x] <- sum(dint1, dint2, dint3)
}
  
RF_refined_calibrated_scatterplot <- ggplot() + theme(panel.grid.minor = element_blank()) + 
  labs(x = "Calibrated score", y = "RF-imputed score") +
  #scale_x_log10(limits = c(0.01,1.1), expand = c(0,0)) +
  geom_histogram(data = complete_frame4_repeats, aes(x = noderer), bins = 20) +
  facet_grid(rows = vars(dint_repeats), cols = vars(nt_repeats), scales = "free_y") +
  NULL; RF_refined_calibrated_scatterplot
...
```{r}
clinvar <- read.csv(file = "Data/ClinVar/250305_clinvar_kozak_mutations.csv", header = T, stringsAsFactors = F)
clinvar_old <- read.delim(file = "Data/ClinVar/ClinVar_5primeUTR.txt", header = F, stringsAsFactors = F, sep = "\t")
...
colnames(clinvar)[colnames(clinvar) == "GeneSymbol"] <- "Symbol"
colnames(clinvar)[colnames(clinvar) == "mutation"] <- "NucleotideChange"
...
Clinvar_Kozak_permutation_fold_densityplot <- ggplot() + theme(panel.grid.minor = element_blank()) + 
  ...
  geom_density(data = transcripts_john_uniq_snv, aes(x = fold_diff, y = ..density..), fill = "red", alpha = 0.3, adjust = 0.5) +
...
Clinvar_Kozak_permutation_fold_densityplot <- ggplot() + theme(panel.grid = element_blank()) + 
  ...
  geom_histogram(data = transcripts_john_uniq_snv, aes(x = fold_diff, y = ..count..), fill = "white", alpha = 0, adjust = 0.8, color = "grey50") +
  geom_histogram(data = transcripts_john_uniq_snv %>% filter(fold_diff <= 0.25), aes(x = fold_diff, y = ..count..), fill = "red", alpha = 1, adjust = 0.8, color = "black") +
