...
```{r Creating custom functions to be used for the analyses}
...
sort3 <- fourWaySortAnalysis(c(99,99,100,100,101,101,102,102)) %>% arrange(desc(w_ave))
sort4 <- fourWaySortAnalysis(c(104,104,105,105,106,106,107,107)) %>% arrange(desc(w_ave))
sort5 <- fourWaySortAnalysis(c(89,89,90,90,91,91,92,92)) %>% arrange(desc(w_ave))
...
sort_controls <- melt(rbind(
  ...
  data.frame(t(t((sort3 %>% filter(sequence == "TTATGGATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])/sum(t((sort3 %>% filter(sequence == "TTATGGATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])))) %>% mutate(replicate = "rep3", sequence = "TTATGGATG"),
  data.frame(t(t((sort4 %>% filter(sequence == "TTATGGATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])/sum(t((sort4 %>% filter(sequence == "TTATGGATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])))) %>% mutate(replicate = "rep4", sequence = "TTATGGATG"),
  data.frame(t(t((sort5 %>% filter(sequence == "TTATGGATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])/sum(t((sort5 %>% filter(sequence == "TTATGGATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])))) %>% mutate(replicate = "rep5", sequence = "TTATGGATG"),
  ...
  data.frame(t(t((sort3 %>% filter(sequence == "GCCACCATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])/sum(t((sort3 %>% filter(sequence == "GCCACCATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])))) %>% mutate(replicate = "rep3", sequence = "GCCACCATG"),
  data.frame(t(t((sort4 %>% filter(sequence == "GCCACCATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])/sum(t((sort4 %>% filter(sequence == "GCCACCATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])))) %>% mutate(replicate = "rep4", sequence = "GCCACCATG"),
  data.frame(t(t((sort5 %>% filter(sequence == "GCCACCATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])/sum(t((sort5 %>% filter(sequence == "GCCACCATG"))[,c("bin1_freq","bin2_freq","bin3_freq","bin4_freq")])))) %>% mutate(replicate = "rep5", sequence = "GCCACCATG")))
...
sort_combined <- merge(sort_combined, sort5[,c("sequence","w_ave","total_count")], by = "sequence", all = T)
colnames(sort_combined) <- c("sequence","sort1","sort1_count","sort2","sort2_count","sort3","sort3_count","sort4","sort4_count","sort5","sort5_count")
sort_combined$total_count <- rowSums(sort_combined[,c("sort1_count","sort2_count","sort3_count","sort4_count","sort5_count")], na.rm = T)
...
```{r Now I'm going to turn weighted average values less than the cutoff value to NA}
...
paste("There are originally this many non-NA score1 weighted averages: values",sum(!is.na(sort_combined2$sort2)))
for(x in 1:nrow(sort_combined2)){
...
paste("There are originally this many non-NA score4 weighted averages: values",sum(!is.na(sort_combined2$sort3)))
for(x in 1:nrow(sort_combined2)){
  if(!(is.na(sort_combined2$sort3_count[x]))){
    if(sort_combined2$sort3_count[x] <= 6){
...
paste("There are this many non-NA score4 weighted averages after the count-based filtering: values",sum(!is.na(sort_combined2$sort3)))

sum(sort_combined2$sort4_count, na.rm = T)
paste("There are originally this many non-NA score3 weighted averages: values",sum(!is.na(sort_combined2$sort4)))
for(x in 1:nrow(sort_combined2)){
...
paste("There are this many non-NA score5 weighted averages after the count-based filtering: values",sum(!is.na(sort_combined2$sort4)))

sum(sort_combined2$sort5_count, na.rm = T)
paste("There are originally this many non-NA score3 weighted averages: values",sum(!is.na(sort_combined2$sort5)))
for(x in 1:nrow(sort_combined2)){
  if(!(is.na(sort_combined2$sort5_count[x]))){
    if(sort_combined2$sort5_count[x] <= 7){
    sort_combined2$sort5[x] <- NA
    }
  }
  if(!(is.na(sort_combined2$sort5[x]))){
    if(sort_combined2$sort5[x] == 0 | sort_combined2$sort5[x] == 1){
      sort_combined2$sort5[x] <- NA
    }
  }
}
paste("There are this many non-NA score5 weighted averages after the count-based filtering: values",sum(!is.na(sort_combined2$sort5)))
```
...
sort_combined2$sd <- 0
for(x in 1:nrow(sort_combined2)){
  sort_combined2$sd[x] <- sd(sort_combined2[x,c("sort1","sort2","sort3","sort4","sort5")], na.rm = T)
}
...
melt_sort <- melt(sort_combined2[,c("sequence","sort1","sort2","sort3","sort4","sort5")], id = "sequence") %>% mutate(n = NA)
...
Individual_vs_4way_sort_scatterplot <- ggplot() + theme(panel.grid = element_blank()) +
  scale_x_log10(limits = c(0.3,0.7)) + scale_y_log10(limits = c(0.0065,1.5)) + 
...
Individual_vs_4way_sort_scatterplot2 <- ggplot() + theme(panel.grid = element_blank()) +
  scale_x_continuous(limits = c(-0.55, 0.-0.15)) +
...
complete_frame3$calibrated_score <- L/(1+exp(-k*(log10(complete_frame3$mfi_lm_norm) - xnot))) + 0.001
...
ggplot() + scale_y_log10(limits = c(0.003,1)) + scale_x_log10(limits = c(0.003,2)) + 
...
round(cor(log10(complete_frame3$mfi_lm_norm), log10(complete_frame3$mfi_individual), method = "pearson", use = "complete"),3),"Spearman's:",
round(cor(log10(complete_frame3$mfi_lm_norm), log10(complete_frame3$mfi_individual), method = "spearman", use = "complete"),3))
...
round(cor(log10(complete_frame3$calibrated_score), log10(complete_frame3$mfi_individual), method = "pearson", use = "complete"),3),"Spearman's:",
round(cor(log10(complete_frame3$calibrated_score), log10(complete_frame3$mfi_individual), method = "spearman", use = "complete"),3))
```
...
density_4way_sort_calcd_plot <- ggplot() + theme_bw() +theme(panel.grid.minor = element_blank()) + 
  scale_x_log10(limits = c(0.01,1.5)) + 
...
#combined_plot <- density_4way_sort_calcd_plot / Individual_vs_4way_sort_calcd_scatterplot
#combined_plot

#ggsave(file = "Plots/4way_sort_calcd_density_scatterplot1.pdf", combined_plot, height = 6, width = 4)
#ggsave(file = "Plots/4way_sort_calcd_density_scatterplot2.pdf", combined_plot, height = 2.5, width = 4)
```
...
PWM_pointplot
ggsave(file = "plots/PWM_pointplot.pdf", PWM_pointplot, height = 1.4, width = 3)
...
RF_refined_noderer_scatterplot <- ggplot() + theme(panel.grid.minor = element_blank()) + 
  ...
  NULL
RF_refined_noderer_scatterplot
...
RF_refined_calibrated_scatterplot <- ggplot() + theme(panel.grid.minor = element_blank()) + 
  labs(x = "Calibrated score", y = "Noderer score") +
  ...
  NULL
RF_refined_calibrated_scatterplot
...
paste("Correlation coefficients between RF-refined scores and the Noderer scores -->","Pearson's r^2:",
...
ggplot() + scale_x_log10() + scale_y_log10() + 
  geom_point(data = complete_frame4_final2_acevedo2, aes(x = acevedo, calibrated_score))
...
```{r Figure out biggest effect size differences possible through snv for each possible Kozak}
transcripts_john <- read.csv(file = "Data/genome_proteome/Kozaks_Improved.csv") 

transcripts_john_uniq <- transcripts_john %>% select(Gene,Kozak) %>% distinct() %>% mutate(kozak = paste0(Kozak,"ATG"))

transcripts_john_uniq_snv <- merge(transcripts_john_uniq, running_snv_df, by = "kozak") %>% mutate(gene = Gene)

Genome_Kozak_permutation_fold_densityplot <- ggplot() + theme(panel.grid.minor = element_blank()) + 
  scale_x_log10(breaks = c(0.01,0.1,1,10,100)) + 
  labs(x = "Fold change from reference Kozak", y = "Density") + 
  geom_hline(yintercept = 0) +
  geom_density(data = transcripts_john_uniq_snv, aes(x = fold_diff), color = "black")
Genome_Kozak_permutation_fold_densityplot
ggsave(file = "Plots/Genome_Kozak_permutation_fold_densityplot.pdf", Genome_Kozak_permutation_fold_densityplot, height = 1.5, width = 3)
```

```{r}
clinvar <- read.delim(file = "Data/ClinVar/ClinVar_5primeUTR.txt", header = F, stringsAsFactors = F, sep = "\t")
colnames(clinvar) <- c("Symbol","GeneID","VariationID","AlleleID","Type","Assembly","NucleotideExpression","NucleotideChange","ProteinExpression","ProteinChange","UsedForNaming","Submitted","OnRefSeqGene")

clinvar2 <- clinvar[,c("Symbol","NucleotideChange")]
clinvar2$nchar <- lapply(clinvar2$NucleotideChange,nchar)
clinvar2b <- clinvar2 %>% filter(nchar == 7 & Symbol != "-")
clinvar2b$gsub_nchar <- nchar(gsub("[A-Z]","",clinvar2b$NucleotideChange))
clinvar2b$gsub_pos <- gsub("[^0-9]","",clinvar2b$NucleotideChange)
clinvar3 <- clinvar2b %>% filter(gsub_nchar == 5 & gsub_pos <= 6) %>% distinct()
clinvar3 <- clinvar3[,c("Symbol","NucleotideChange")]
colnames(clinvar3) <- c("gene","hgvs_conseq")
paste("The number of Kozak variants found in ClinVar is:",nrow(clinvar3))

transcripts_john_uniq_clinvar <- merge(transcripts_john_uniq_snv, clinvar3[,c("gene","hgvs_conseq")], by = c("gene","hgvs_conseq")) 
transcripts_john_uniq_clinvar2 <- transcripts_john_uniq_clinvar %>% group_by(gene, hgvs_conseq) %>% summarize(n = n())
transcripts_john_uniq_clinvar3 <- merge(transcripts_john_uniq_clinvar,transcripts_john_uniq_clinvar2, by = c("gene","hgvs_conseq")) %>% distinct() %>% arrange(fold_diff)

transcripts_john_uniq_clinvar3_multikozak <- transcripts_john_uniq_clinvar3[,c("gene","kozak")] %>% group_by(gene, kozak) %>% distinct() %>% mutate(count = 1) %>% group_by(gene) %>% summarize(sum_count = sum(count)) %>% filter(sum_count > 1)
...
transcripts_acmg <- transcripts_john_uniq_clinvar3 %>% filter(gene %in% acmg73_list)
transcripts_acmg$gene_kozak <- paste0(transcripts_acmg$gene,"_",transcripts_acmg$kozak)

transcripts_acmg_most_common <- data.frame(table(transcripts_acmg$gene)) %>% arrange(desc(Freq))

Clinvar_Kozak_permutation_fold_densityplot <- ggplot() + theme(panel.grid.minor = element_blank()) + 
  scale_x_log10(breaks = c(0.01,0.1,1,10,100)) + 
  labs(x = "Fold change from reference Kozak", y = "Density") + 
  geom_hline(yintercept = 0) +
  geom_histogram(data = running_snv_df, aes(x = fold_diff, y = ..density..), fill = "black", alpha = 0.3) +
  geom_histogram(data = transcripts_john_uniq_snv, aes(x = fold_diff, y = ..density..), fill = "blue", alpha = 0.3) +
  geom_histogram(data = transcripts_acmg, aes(x = fold_diff, y = ..density..), fill = "red", alpha = 0.3)
Clinvar_Kozak_permutation_fold_densityplot
...
transcripts_john_uniq_clinvar_max_diffs <- transcripts_john_uniq_clinvar %>% group_by(Gene) %>% summarize(max_fold_diff = max(fold_diff))

sorted_permutations <- sort(running_snv_df$fold_diff)
sorted_permutations_ranks <- rank(sorted_permutations)
sorted_permutations_cdf <- sorted_permutations_ranks / length(sorted_permutations)
sorted_permutations_cdf_data <- data.frame(x = sorted_permutations, y = sorted_permutations_cdf)

sorted_kozak_permutations <- sort(transcripts_john_uniq_snv$fold_diff)
sorted_kozak_permutations_ranks <- rank(sorted_kozak_permutations)
sorted_kozak_permutations_cdf <- sorted_kozak_permutations_ranks / length(sorted_kozak_permutations)
sorted_kozak_permutations_cdf_data <- data.frame(x = sorted_kozak_permutations, y = sorted_kozak_permutations_cdf)

sorted_kozak_clinvar_permutations <- sort(transcripts_john_uniq_clinvar3$fold_diff)
sorted_kozak_clinvar_permutations_ranks <- rank(sorted_kozak_clinvar_permutations)
sorted_kozak_clinvar_permutations_cdf <- sorted_kozak_clinvar_permutations_ranks / length(sorted_kozak_clinvar_permutations)
sorted_kozak_clinvar_permutations_cdf_data <- data.frame(x = sorted_kozak_clinvar_permutations, y = sorted_kozak_clinvar_permutations_cdf)

sorted_permutations_cdf_data2 <- sorted_permutations_cdf_data %>%
  group_by(row_group = ceiling(row_number() / 10)) %>%
  summarize_all(.funs = ~mean(., na.rm = TRUE)) %>%
  ungroup()

sorted_kozak_permutations_cdf_data2 <- sorted_kozak_permutations_cdf_data %>%
  group_by(row_group = ceiling(row_number() / 100)) %>%
  summarize_all(.funs = ~mean(., na.rm = TRUE)) %>%
  ungroup()

Cumulative_probability_graph <- ggplot() + theme(panel.grid = element_blank()) +
  scale_y_log10(expand = c(0,0)) + scale_x_continuous(expand = c(0,0)) +
  geom_step(data = sorted_permutations_cdf_data2, aes(y, x), direction = "mid", size = 2, alpha = 0.9) +
  #geom_step(data = sorted_kozak_permutations_cdf_data2, aes(y, x), direction = "mid", color = "cyan", size = 2, alpha = 0.6) +
  geom_step(data = sorted_kozak_clinvar_permutations_cdf_data, aes(y, x), direction = "mid", color = "red", size = 2, alpha = 0.9) +
  labs(y = "Fold difference", x = "Cumulative Probability") +
  geom_hline(yintercept = 0.25, linetype = 1, alpha = 0.2) +
  geom_vline(xintercept = 0.052, linetype = 1, alpha = 0.2)
Cumulative_probability_graph
ggsave(file = "Plots/Cumulative_probability_graph.pdf", Cumulative_probability_graph, height = 1.4, width = 3)

paste("The fraction of ClinVar Kozak variants that are expected to reduce the steady-state abundance of the protein by 4-fold or more:",round(max(sorted_kozak_clinvar_permutations_cdf_data[sorted_kozak_clinvar_permutations_cdf_data$x < 0.25,"y"]),3))

nrow(subset(transcripts_john_uniq_clinvar3, fold_diff <= 0.25))
nrow(subset(transcripts_john_uniq_clinvar3, fold_diff <= 0.25)) / nrow(transcripts_john_uniq_clinvar3)
```


```{r}
acmg_manual_kozaks <- read.csv(file = "Data/ClinVar/ACMG73_manual_kozaks.csv", header = T, stringsAsFactors = F)
acmg_manual_kozaks$gene_kozak <- paste0(acmg_manual_kozaks$gene,"_",acmg_manual_kozaks$kozak)

transcripts_acmg2 <- transcripts_acmg %>% filter(gene_kozak %in% acmg_manual_kozaks$gene_kozak)

transcripts_acmg2_genetable <- data.frame(table(transcripts_acmg2$gene)) %>% arrange(desc(Freq))

transcripts_acmg2_genetable_2plus <- transcripts_acmg2_genetable %>% filter(Freq >= 2)
transcripts_acmg2_genetable_2plus_levels <- transcripts_acmg2_genetable_2plus$Var1

transcripts_acmg2_2plus <- transcripts_acmg2 %>% filter(gene %in% transcripts_acmg2_genetable_2plus$Var1)
transcripts_acmg2_2plus$gene <- factor(transcripts_acmg2_2plus$gene, levels = rev(transcripts_acmg2_genetable_2plus_levels))

transcripts_acmg2_2plus$label <- substr(transcripts_acmg2_2plus$hgvs_conseq, 3, 12)

Most_clinvar_grouped_scatterplot <- ggplot() + 
  theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank()) + 
  labs(x = "Fold abundance of reference Kozak", y = NULL) +
  scale_x_log10() + scale_y_discrete(expand = c(0,1)) +
  geom_vline(xintercept = 1, linetype = 2, alpha = 0.4) +
  geom_point(data = transcripts_acmg2_2plus, aes(x = fold_diff, y = gene), alpha = 0.25) +
  geom_text(data = transcripts_acmg2_2plus %>% filter(fold_diff < 0.25), 
            aes(x = fold_diff, y = gene, label = label), angle = 0, hjust = 0, vjust = -0.25, color = "red", size = 2) +
  NULL; Most_clinvar_grouped_scatterplot
ggsave(file = "Plots/Most_clinvar_grouped_scatterplot.pdf", Most_clinvar_grouped_scatterplot, height = 4, width = 3.5)
```

```{r}
paste("BRCA1 Reference calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "AAAGAAATG","calibrated_score"],2))
paste("BRCA1 VCV000869050 calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "AATGAAATG","calibrated_score"],2))

paste("MSH6 Reference calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "GTCGGTATG","calibrated_score"],2))
paste("MSH6 VCV001736915 calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "GTCTGTATG","calibrated_score"],2))

paste("PMS2 Reference calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "GCATCCATG","calibrated_score"],2))
paste("PMS2 VCV001798654 calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "GCATGCATG","calibrated_score"],2))
paste("PMS2 VCV000822528 calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "GCATACATG","calibrated_score"],2))

paste("SDHD Reference calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "AACGAGATG","calibrated_score"],2))
paste("SDHD VCV000619913 calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "AATGAGATG","calibrated_score"],2))

paste("RAD51C Reference calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "CCTGCGATG","calibrated_score"],2))
paste("RAD51C VCV000824488 calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "CCTTCGATG","calibrated_score"],2))
paste("RAD51C VCV000824488 calibrated_score score:",round(complete_frame4[complete_frame4$sequence == "CATGCGATG","calibrated_score"],2))
```


```{r}
## Below are manually confirming potentially problematic VUSses
transcripts_acmg$comment <- NA
transcripts_acmg[transcripts_acmg$gene == "MLH1" & transcripts_acmg$hgvs_conseq == "c.-3A>T" & transcripts_acmg$kozak == "GCCAAAATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "MSH6" & transcripts_acmg$hgvs_conseq == "c.-3G>T" & transcripts_acmg$kozak == "GTCGGTATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "LMNA" & transcripts_acmg$hgvs_conseq == "c.-5C>A" & transcripts_acmg$kozak == "CCGGCCATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "RAD51C" & transcripts_acmg$hgvs_conseq == "c.-3G>T" & transcripts_acmg$kozak == "CCTGCGATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "RAD51C" & transcripts_acmg$hgvs_conseq == "c.-3G>C" & transcripts_acmg$kozak == "CCTGCGATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "KCNQ1" & transcripts_acmg$hgvs_conseq == "c.-3G>C" & transcripts_acmg$kozak == "CTCGTTATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "SDHD" & transcripts_acmg$hgvs_conseq == "c.-4C>T" & transcripts_acmg$kozak == "AACGAGATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "MUTYH" & transcripts_acmg$hgvs_conseq == "c.-3A>C" & transcripts_acmg$kozak == "GCCATCATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "SDHA" & transcripts_acmg$hgvs_conseq == "c.-3G>T" & transcripts_acmg$kozak == "GCAGACATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "PMS2" & transcripts_acmg$hgvs_conseq == "c.-2C>G" & transcripts_acmg$kozak == "GCATCCATG","comment"] <- "VUS that is likely pathogenic"
transcripts_acmg[transcripts_acmg$gene == "PALB2" & transcripts_acmg$hgvs_conseq == "c.-2C>G" & transcripts_acmg$kozak == "TGCCCGATG","comment"] <- "VUS that is likely pathogenic"

## Now just all possible Kozak changes of ACMG73 genes

transcripts_john_uniq_acmg <- merge(transcripts_john_uniq_snv, acmg_manual_kozaks, by = c("gene","kozak")) %>% distinct() %>% arrange(fold_diff)

```

```{r Bringing in individually exported files}

# Get the list of files in the folder
folder_path <- "Data/ClinVar/Genes_UTR"  # Replace with the actual folder path
files <- list.files(folder_path, pattern = "\\.txt$", full.names = TRUE)

# Initialize an empty data table to store the combined rows
combined_data <- data.table()

# Import and bind all rows from each file
for (file in files) {
  data <- fread(file, sep = "\t")
  combined_data <- rbind(combined_data, data)
}

combined_data$nchar <- nchar(combined_data$Name)
combined_data2 <- combined_data %>% filter(nchar <= 29)

clinvar_pos_extractor <- function(text){
  gsub("[^-0-9]","",strsplit(text,"c.")[[1]][2])
}

combined_data2$pos <- lapply(combined_data2$Name,clinvar_pos_extractor)
combined_data3 <- combined_data2 %>% filter(pos %in% seq(-6,-1,1))

clinvar_mut_extractor <- function(text){
  gsub("[^>A-Z]","",strsplit(text,"c.")[[1]][2])
}

combined_data3$mut <- lapply(combined_data3$Name,clinvar_mut_extractor)

combined_data3$hgvs_conseq <- paste0("c.",combined_data3$pos,combined_data3$mut)

combined_data4 <- combined_data3[,c("Gene(s)","hgvs_conseq","Condition(s)","Accession","Clinical significance (Last reviewed)")]
colnames(combined_data4)[1] <- "gene"

## Now merge with prev dataset

transcripts_john_uniq_acmg_clinvar <- merge(transcripts_john_uniq_acmg, combined_data4, by = c("gene","hgvs_conseq")) %>% arrange(fold_diff)

table(transcripts_john_uniq_acmg_clinvar$gene)
```

```{r}
## 
rad51c_orig_wt <- subset(complete_frame4, sequence %in% c("CCTGCGATG","CCTTCGATG"))

ggplot() + 
  geom_point(data = rad51c_orig_wt, aes(x = "Rad51C_orig", y = sort_geomean[1])) + 
  geom_errorbar(data = rad51c_orig_wt, aes(x = "Rad51C_orig", ymin = sort_lower_conf[1], ymax = sort_upper_conf[1])) +
  geom_point(data = rad51c_orig_wt, aes(x = "Rad51C_mut", y = sort_geomean[2])) + 
  geom_errorbar(data = rad51c_orig_wt, aes(x = "Rad51C_mut", ymin = sort_lower_conf[2], ymax = sort_upper_conf[2]))
...
Scaled_individual_multiplex_s2_plot <- ggplot() + theme(panel.grid = element_blank()) + 
  scale_x_log10(limits = c(0.01,2), expand = c(0,0)) + scale_y_continuous(limits = c(0,1)) +#scale_y_log10(limits = c(0.008,10)) + 
  ...
  NULL
Scaled_individual_multiplex_s2_plot
...
Scaled_individual_multiplex_s1_plot <- ggplot() + theme(panel.grid = element_blank()) + 
  ...
  NULL
Scaled_individual_multiplex_s1_plot
...
```{r BtKY72 kozak and sequence rep1}
#  Rep 1
 sa1_bt_low <- read.delim(file = myfiles[161], sep = "\t")
 sa1_bt_high <- read.delim(file = myfiles[162], sep = "\t")
 sa1_bt_count <- merge(sa1_bt_low, sa1_bt_high, by = "X", all.x = T)
 colnames(sa1_bt_count) <- c("kozak","low","high")
 
 ggplot() + scale_x_log10() + scale_y_log10() + 
   geom_point(data = sa1_bt_count, aes(x = low, y = high), alpha = 0.2)
 
 sa1_bt <- makeExperimentFrame2(c(153,154,161,162)) %>% filter(h_enrichment != "Inf") %>% arrange(desc(h_enrichment))
 sa1_bt <- merge(sa1_bt, complete_frame6[,c("sequence","calibrated_score")])
 sa1_bt$n1 <- substr(sa1_bt$sequence,1,1)
 sa1_bt <- merge(sa1_bt, kozak_and_variant_key, by = "n1", all.x = T)
 sa1_bt_template_henrichment <- (sa1_bt %>% filter(sequence == "XXXXXXXXX"))$h_enrichment
 sa1_bt <- sa1_bt %>% filter(variant %in% c("WT","I21N","D355N"))
 sa1_bt$variant <- factor(sa1_bt$variant, levels = c("WT","I21N","K31D","D355N"))
 
 ggplot() + scale_x_log10() + scale_y_log10() + labs(x = "Calibrated abundance level", y = "Rate of infection (enrichment upon hygro treatment)") +
   geom_point(data = sa1_bt, aes(x = calibrated_score, y = h_enrichment, color = variant), alpha = 0.2) +
   facet_grid(cols = vars(variant))
 
#  Rep 2
 sa2_bt_low <- read.delim(file = myfiles[177], sep = "\t")
 sa2_bt_high <- read.delim(file = myfiles[178], sep = "\t")
 sa2_bt_count <- merge(sa2_bt_low, sa2_bt_high, by = "X", all.x = T)
 colnames(sa2_bt_count) <- c("kozak","low","high")
 
 ggplot() + scale_x_log10() + scale_y_log10() + 
   geom_point(data = sa2_bt_count, aes(x = low, y = high), alpha = 0.2)
 
 sa2_bt <- makeExperimentFrame2(c(169,170,177,178)) %>% filter(h_enrichment != "Inf") %>% arrange(desc(h_enrichment))
 sa2_bt <- merge(sa2_bt, complete_frame6[,c("sequence","calibrated_score")])
 sa2_bt$n1 <- substr(sa2_bt$sequence,1,1)
 sa2_bt <- merge(sa2_bt, kozak_and_variant_key, by = "n1", all.x = T)
 sa2_bt_template_henrichment <- (sa2_bt %>% filter(sequence == "XXXXXXXXX"))$h_enrichment
 sa2_bt <- sa2_bt %>% filter(variant %in% c("WT","I21N","D355N"))
 sa2_bt$variant <- factor(sa2_bt$variant, levels = c("WT","I21N","K31D","D355N"))
 
 ggplot() + scale_x_log10() + scale_y_log10() + labs(x = "Calibrated abundance level", y = "Rate of infection (enrichment upon hygro treatment)") +
   geom_point(data = sa2_bt, aes(x = calibrated_score, y = h_enrichment, color = variant), alpha = 0.2) +
   facet_grid(cols = vars(variant))
 
#  Now average the values
 sa_bt <- merge(sa1_bt[,c("sequence","calibrated_score","variant","h_enrichment")], sa2_bt[,c("sequence","h_enrichment")], by = "sequence")
 sa_bt$h_enrichment.x2 <- sa_bt$h_enrichment.x / mean((sa_bt %>% filter(variant == "WT" & calibrated_score < 1.1 & calibrated_score > 0.9))$h_enrichment.x, na.rm = T)
 sa_bt$h_enrichment.y2 <- sa_bt$h_enrichment.y / mean((sa_bt %>% filter(variant == "WT" & calibrated_score < 1.1 & calibrated_score > 0.9))$h_enrichment.y, na.rm = T)
 sa_bt$ave_enrichment <- (sa_bt$h_enrichment.x2 + sa_bt$h_enrichment.y2)/2
 sa_bt_template_henrichment <- (sa1_bt_template_henrichment + sa2_bt_template_henrichment)/2
 
 ggplot() + scale_x_log10() + scale_y_log10(limits = c(0.003,3)) + labs(x = "Calibrated abundance level", y = "Rate of infection (enrichment upon hygro treatment)") +
   geom_hline(yintercept = sa_bt_template_henrichment, color = "purple", size = 2, alpha = 0.5) +
   geom_point(data = sa_bt, aes(x = calibrated_score, y = ave_enrichment, color = variant), alpha = 0.2) +
   facet_grid(cols = vars(variant))
 
#  Whole thing is very noisy....
```

```{r Khosta2 kozak and sequence rep1}
## Rep 1
sa1_k2_low <- read.delim(file = myfiles[163], sep = "\t")
sa1_k2_high <- read.delim(file = myfiles[164], sep = "\t")
sa1_k2 <- merge(sa1_k2_low, sa1_k2_high, by = "X", all.x = T)
colnames(sa1_k2) <- c("kozak","low","high")

ggplot() + scale_x_log10() + scale_y_log10() + 
  geom_point(data = sa1_k2, aes(x = low, y = high), alpha = 0.2)
## 164 is missing most of its data

sa1_k2 <- makeExperimentFrame2(c(153,154,163,163)) %>% filter(h_enrichment != "Inf") %>% arrange(desc(h_enrichment))
sa1_k2 <- merge(sa1_k2, complete_frame6[,c("sequence","calibrated_score")])
sa1_k2$n1 <- substr(sa1_k2$sequence,1,1)
sa1_k2 <- merge(sa1_k2, kozak_and_variant_key, by = "n1", all.x = T)
sa1_k2_template_henrichment <- (sa1_k2 %>% filter(sequence == "XXXXXXXXX"))$h_enrichment
sa1_k2 <- sa1_k2 %>% filter(variant %in% c("WT","I21N","D355N"))
sa1_k2$variant <- factor(sa1_k2$variant, levels = c("WT","I21N","K31D","D355N"))

ggplot() + scale_x_log10() + scale_y_log10() + labs(x = "Calibrated abundance level", y = "Rate of infection (enrichment upon hygro treatment)") +
  geom_point(data = sa1_k2, aes(x = calibrated_score, y = h_enrichment, color = variant), alpha = 0.2) +
  facet_grid(cols = vars(variant))

## Rep 2
sa2_k2_low <- read.delim(file = myfiles[179], sep = "\t")
sa2_k2_high <- read.delim(file = myfiles[180], sep = "\t")
sa2_k2 <- merge(sa2_k2_low, sa2_k2_high, by = "X", all.x = T)
colnames(sa2_k2) <- c("kozak","low","high")

ggplot() + scale_x_log10() + scale_y_log10() + 
  geom_point(data = sa2_k2, aes(x = low, y = high), alpha = 0.2)
## 164 is missing most of its data

sa2_k2 <- makeExperimentFrame2(c(169,170,179,180)) %>% filter(h_enrichment != "Inf") %>% arrange(desc(h_enrichment))
sa2_k2 <- merge(sa2_k2, complete_frame6[,c("sequence","calibrated_score")])
sa2_k2$n1 <- substr(sa2_k2$sequence,1,1)
sa2_k2 <- merge(sa2_k2, kozak_and_variant_key, by = "n1", all.x = T)
sa2_k2_template_henrichment <- (sa2_k2 %>% filter(sequence == "XXXXXXXXX"))$h_enrichment
sa2_k2 <- sa2_k2 %>% filter(variant %in% c("WT","I21N","D355N"))
sa2_k2$variant <- factor(sa2_k2$variant, levels = c("WT","I21N","K31D","D355N"))

ggplot() + scale_x_log10() + scale_y_log10() + labs(x = "Calibrated abundance level", y = "Rate of infection (enrichment upon hygro treatment)") +
  geom_point(data = sa2_k2, aes(x = calibrated_score, y = h_enrichment, color = variant), alpha = 0.2) +
  facet_grid(cols = vars(variant))

## Now average the values
sa_k2 <- merge(sa1_k2[,c("sequence","calibrated_score","variant","h_enrichment")], sa2_k2[,c("sequence","h_enrichment")], by = "sequence")
sa_k2$h_enrichment.x2 <- sa_k2$h_enrichment.x / mean((sa_k2 %>% filter(variant == "WT" & calibrated_score < 1.1 & calibrated_score > 0.9))$h_enrichment.x, na.rm = T)
sa_k2$h_enrichment.y2 <- sa_k2$h_enrichment.y / mean((sa_k2 %>% filter(variant == "WT" & calibrated_score < 1.1 & calibrated_score > 0.9))$h_enrichment.y, na.rm = T)
sa_k2$ave_enrichment <- (sa_k2$h_enrichment.x2 + sa_k2$h_enrichment.y2)/2
sa_k2 <- sa_k2 %>% filter(variant %in% c("WT","I21N","D355N"))
sa_k2_template_henrichment <- (sa1_k2_template_henrichment + sa2_k2_template_henrichment)/2

ggplot() + scale_x_log10() + scale_y_log10(limits = c(0.003,3)) + labs(x = "Calibrated abundance level", y = "Rate of infection (enrichment upon hygro treatment)") +
  geom_hline(yintercept = sa_k2_template_henrichment, color = "purple", size = 2, alpha = 0.5) +
  geom_point(data = sa_k2, aes(x = calibrated_score, y = ave_enrichment, color = variant), alpha = 0.2) +
  facet_grid(cols = vars(variant))
```
...
```{r Performing LOESS off minilibrary data}
loess_span <- 0.75
...
## Now bringing in the mix of various cDNAs
...
```{r Calculate frequencies and enrichment scores}
...
```{r Compare frequencies across datasets}
ggplot() + scale_x_log10() + scale_y_log10() + 
...
```{r Filtering the data and recalculating frequencies}
...
```{r Compare frequencies across datasets after filtering}
ggplot() + scale_x_log10() + scale_y_log10() + 
