...
clinvar <- read.csv(file = "Data/ClinVar/250305_clinvar_kozak_mutations.csv", header = T, stringsAsFactors = F) %>% filter(mutation_location == "kozak" & !(old_kozak %in% c("error:no_kozak_found", "error:mutation_not_match_kozak", "error:microsatellite")))
...
Most_clinvar_grouped_scatterplot <- ggplot() + 
  ...
  geom_label_repel(data = clinvar2_acmg_3plus %>% filter(fold_diff < 0.2 & mut_kozak %in% clinvar_confidently_low_mutkozak_list), 
