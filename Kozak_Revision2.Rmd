...
pairwise_fold_diff_colors <- c("A/C" = "green", "A/G" = "dodgerblue1", "A/T" = "darkorchid1")

Pairwise_fold_diff <- ggplot() + 
  scale_fill_manual(values = pairwise_fold_diff_colors) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "top") + 
  ...
  geom_jitter(data = pairwise_fold_diff, aes(x = position, y = values, fill = subset, color = NA), 
              position  = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.5), stat = "identity", shape = 21, size = 0.5, alpha = 0.05) + 
  geom_point(data = pairwise_summary_fold_diff, aes(x = position, y = geomean, color = subset, fill = subset), 
             position  = position_dodge(width = 0.5), stat = "identity", shape = 45, size = 3, alpha = 1, color = "black") +
  NULL
Pairwise_fold_diff
...
paste("The total number of variants that we scored was", nrow(complete_frame4))
...
Avecedo_scatterplot <- ggplot() + theme(panel.grid = element_blank()) + 
  ...
  geom_smooth(data = complete_frame4_final2_acevedo2, aes(x = acevedo, y = calibrated_score), alpha = 0) +
...
Ambrosini_scatterplot <- ggplot() + theme(panel.grid = element_blank()) +
  ...
  scale_x_continuous(limits = c(0.25, 0.65), expand = c(0,0)) + 
  scale_y_continuous(expand = c(0,0)) +
  geom_point(data = complete_frame4_final2_ambrosini, aes(x = ambrosini, calibrated_score), alpha = 0.4) +
  geom_smooth(data = complete_frame4_final2_ambrosini, aes(x = ambrosini, y = calibrated_score), alpha = 0) +
...
snv_analysis <- complete_frame4[,c("sequence","calibrated_score","sort_upper_conf","sort_lower_conf")]

running_snv_df <- data.frame("orig_sequence" = NA,"sequence" = NA, "hgvs_conseq" = NA, "calibrated_score" = NA, "sort_upper_conf" = NA,"sort_lower_conf" = NA,"fold_diff" = NA, "conf_nonoverlap" = NA)
for(row in 1:nrow(snv_analysis)){
  temp_wt_seq <- snv_analysis$sequence[row]
  temp_wt_score <- snv_analysis$calibrated_score[row]
  temp_wt_lower <- snv_analysis$sort_lower_conf[row]
  hgvs_conseq <- c()
  sequence <- c()
  for(pos in 1:6){
    for(nt in c("A","C","G","T")){
  ...
  temp_subset <- merge(temp_combined, snv_analysis, by = "sequence") %>% filter(sequence != temp_wt_seq)
  temp_subset$fold_diff <- temp_subset$calibrated_score / temp_wt_score
  temp_subset$conf_nonoverlap <- temp_subset$sort_upper_conf < temp_wt_lower
...
colnames(running_snv_df) <- c("kozak","mut_kozak","hgvs_conseq","calibrated_score","sort_upper_conf","sort_lower_conf","fold_diff","conf_nonoverlap")
...
clinvar4 <- clinvar2; clinvar4$conf_low <- NA
for(x in 1:nrow(clinvar4)){
  if(!is.na(clinvar4$conf_nonoverlap[x])){
    if(clinvar4$conf_nonoverlap[x] == TRUE & clinvar4$fold_diff[x] <= 0.25){clinvar4$conf_low[x] <- "Yes"}
  }
}
...
write.table(file = "Output_datatables/Supp_table_2_ClinVar.tsv", clinvar4[,5:ncol(clinvar4)], row.names = F, quote = F, sep = "\t")
```

```{r clinvar variants subsetted to acmg59}
clinvar2_acmg_most_common <- data.frame(table(clinvar2_acmg$gene)) %>% arrange(desc(Var1))
...
Most_clinvar_grouped_scatterplot <- ggplot() + 
  ...
  scale_x_log10(limits = c(0.02,18)) + scale_y_discrete(expand = c(0,1), limits=rev) +
...
## Now seeing how the G>C changes in the beta-thallesemia and GATA4 Kozaks alter protein steady-state expression
```{r GATA4 and }

beta_thalassemia <- subset(complete_frame4, sequence %in% c("GACACCATG","CACACCATG"))

gata4 <- subset(complete_frame4, sequence %in% c("GGGACCATG","CGGACCATG"))
...
rf_predict_mfi_vs_SARS2_scatterplot <- ggplot() + scale_x_log10(limits = c(0.01,1.4), expand = c(0,0)) + scale_y_log10(limits = c(0.003,10), expand = c(0,0)) + 
  labs(x = "rf_predict_mfi score", y = "SARS2 infection") +
  geom_point(data= complete_frame5, aes(x = rf_predict_mfi, y = s2), alpha = 0.4) +
  NULL; rf_predict_mfi_vs_SARS2_scatterplot
ggsave(file = "plots/rf_predict_mfi_vs_SARS2_scatterplot.png", rf_predict_mfi_vs_SARS2_scatterplot, height = 4, width = 6)

```

```{r VSV and S2 overlaid plots for presentations}

rf_predict_mfi_s2_vsv_1 <- ggplot() + 
  scale_x_log10(limits = c(0.02,1.15), expand = c(0,0)) + scale_y_log10(limits = c(0.005,5), expand = c(0,0)) + 
  labs(x = "ACE2 abundance", y = "Relative infection score") +
  geom_point(data= complete_frame5 %>% filter(!is.na(s2) & s2 > 0.008), aes(x = rf_predict_mfi, y = s2), alpha = 0.1) +
  geom_smooth(data= complete_frame5 %>% filter(!is.na(s2) & s2 > 0.008), aes(x = rf_predict_mfi, y = s2), alpha = 0, size = 2) +
  NULL; rf_predict_mfi_s2_vsv_1
ggsave(file = "plots/rf_predict_mfi_s2_vsv_1.png", rf_predict_mfi_s2_vsv_1, height = 3, width = 4)

rf_predict_mfi_s2_vsv_2 <- rf_predict_mfi_s2_vsv_1 + 
  geom_point(data= complete_frame5 %>% filter(!is.na(vsv) & vsv > 0.008), aes(x = rf_predict_mfi, y = vsv), alpha = 0.1, color = "red") + 
  geom_smooth(data= complete_frame5 %>% filter(!is.na(vsv) & vsv > 0.008), aes(x = rf_predict_mfi, y = vsv), alpha = 0, size = 2, color = "red") +
  NULL; rf_predict_mfi_s2_vsv_2
ggsave(file = "plots/rf_predict_mfi_s2_vsv_2.png", rf_predict_mfi_s2_vsv_2, height = 3, width = 4)
```
...
write.csv(file = "Output_datatables/Supp_table_4_kozak_sortseq_and_infection.csv", complete_frame6[,c("sequence","sort_geomean","sort_upper_conf","sort_lower_conf","mfi_lm_norm","calibrated_score","mfi_individual","noderer","rf_predict_mfi","s1","s2","vsv","individual_s1","individual_s2","individual_vsv")], row.names = F, quote = F)
...
write.csv(file = "Output_datatables/Supp_table_5_mixed_library_infection.csv", sequence_abundance_compiled, row.names = F, quote = F)
...
stim1_colors <- c("WT" = "red", "R429C" = "blue", "R304W" = "green3")

STIM1_geom_smooth_plot <- ggplot() + 
  theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank()) + 
  scale_x_log10(expand = c(0,0), limits = c(0.007,1.34)) + scale_y_log10(expand = c(0,0)) + 
  scale_color_manual(values = stim1_colors) + 
...
### Revision2 - incorporating Sample et al Ribosomal Loading values
```{r}
ribo_loads <- read.csv(file = "Data/gfp_merge_kozak.csv", header = T) %>% mutate(sequence = paste0(kozak,"ATG"), riboload = (mean.x + mean.y)/2)

complete_frame7 <- merge(complete_frame6, ribo_loads[,c("sequence","mean.x","mean.y","riboload")], by = "sequence", all = T)

ggplot() + geom_point(data = complete_frame7, aes(x = mean.x, y = mean.y), alpha = 0.01)

ggplot() + geom_point(data = complete_frame7, aes(x = log10(calibrated_score), y = riboload), alpha = 0.1) +
  geom_smooth(data = complete_frame7, aes(x = log10(calibrated_score), y = riboload)) +
  scale_y_continuous(limits = c(4.5,6.5))

Calibrated_Sample_comparison <- ggplot() + geom_point(data = complete_frame7, aes(x = log10(calibrated_score), y = riboload), alpha = 0.1) +
  geom_smooth(data = complete_frame7, aes(x = log10(calibrated_score), y = riboload), alpha = 0) +
  scale_y_continuous(limits = c(4.5,6.5)) +
  theme(panel.grid = element_blank()) + 
  labs(x = "Calibrated score", y = "Ribosomal load") +
  NULL; Calibrated_Sample_comparison
ggsave(file = "Plots/Calibrated_Sample_comparison.pdf", Calibrated_Sample_comparison, height = 2, width = 3)

paste("Pearson's r^2 value between our calibrated scores and the Sample et al mean ribosomal loads", round(cor(log10(complete_frame7$calibrated_score), complete_frame7$riboload, method = "pearson", use = "complete")^2,2))
paste("Spearman's rho^2 value between our calibrated scores and the Sample et al mean ribosomal loads", round(cor(complete_frame7$calibrated_score, complete_frame7$riboload, method = "spearman", use = "complete")^2,2))

Imputed_Sample_comparison <- ggplot() + geom_point(data = complete_frame7, aes(x = log10(rf_predict_mfi), y = riboload), alpha = 0.1) +
  geom_smooth(data = complete_frame7, aes(x = log10(rf_predict_mfi), y = riboload), alpha = 0) +
  scale_y_continuous(limits = c(4.5,6.5)) +
  theme(panel.grid = element_blank()) + 
  labs(x = "Random-forest - imputed score", y = "Ribosomal load") +
  NULL; Imputed_Sample_comparison
ggsave(file = "Plots/Imputed_Sample_comparison.pdf", Imputed_Sample_comparison, height = 2, width = 3)

Noderer_Sample_comparison <- ggplot() + geom_point(data = complete_frame7, aes(x = noderer, y = riboload), alpha = 0.1) +
  geom_smooth(data = complete_frame7, aes(x = noderer, y = riboload), alpha = 0) +
  scale_y_continuous(limits = c(4.5,6.5)) +
  theme(panel.grid = element_blank()) + 
  labs(x = "Noderer value", y = "Ribosomal load") +
  NULL; Noderer_Sample_comparison
ggsave(file = "Plots/Noderer_Sample_comparison.pdf", Noderer_Sample_comparison, height = 2, width = 3)

paste("Pearson's r^2 value between the Noderer values and the Sample et al mean ribosomal loads", round(cor(log10(complete_frame7$noderer), complete_frame7$riboload, method = "pearson", use = "complete")^2,2))
paste("Spearman's rho^2 value between the Noderer values and the Sample et al mean ribosomal loads", round(cor(complete_frame7$noderer, complete_frame7$riboload, method = "spearman", use = "complete")^2,2))


ggplot() + geom_point(data = complete_frame7, aes(x = log10(mfi_individual), y = riboload), alpha = 0.6) +
  geom_smooth(data = complete_frame7, aes(x = log10(mfi_individual), y = riboload))
  scale_y_continuous(limits = c(4.5,6.5))


ggplot() + geom_point(data = complete_frame7, aes(x = noderer, y = riboload), alpha = 0.1) + scale_y_continuous(limits = c(4.5,6.5))
ggplot() + geom_point(data = complete_frame7, aes(x = log10(rf_predict_mfi), y = noderer), alpha = 0.2) +
  geom_smooth(data = complete_frame7, aes(x = log10(rf_predict_mfi), y = noderer)) #+ scale_y_continuous(limits = c(4.5,6.5))
